                            ...
                            <template v-for="(v, k) in list" :key="k">
                                <tr class="align-middle">
                                    ...
                                    <td class="align-middle text-center">
                                        ...
                                        <template v-else>
                                            <button v-on:click="doiTrangThai(v)" class="btn btn-danger">Đã Khóa</button>
                                        </template>
                                    </td>
                                    ...
                                    <td class="text-end">
                                        <button class="btn btn-secondary w-100 text-end">
                                         {{ format(v.so_du) }}
                                        </button>
                                    </td>
                                    <td class="text-center">
                                        <button v-on:click="loadLichSu(v)" class="btn btn-success  me-1 w-10"
                                        data-bs-toggle="modal" data-bs-target="#exampleModal">Lich su</button>
                                        <button v-on:click="Object.assign(nap_tien, v)" class="btn btn-warning me-1 w-10"
                                            data-bs-toggle="modal" data-bs-target="#napTienModal">Nạp Tiền</button>
  ...
  <div class="modal-dialog modal-xl">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="exampleModalLabel">Lich su nap tien</h1>
            ...
            <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                <template v-for="(v,index) in listLS" :key="index">
                                    <tr>
    ...
    <div class="modal fade" id="napTienModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="exampleModalLabel">Chức Năng Nạp Tiền</h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
...
export default {
    ...
    methods: {
        ...
        loadLichSu(v) {
            axios
                .post('http://127.0.0.1:8000/api/admin/nap-tien/data-one',v ,{
                    headers: {
                        Authorization: 'Bearer ' + localStorage.getItem("key_admin")
                    }
                })
                .then((res) => {
                    this.listLS = res.data.data;
                })
        },
        xoa() {
            axios
